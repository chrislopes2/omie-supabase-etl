...
def puxar_contas_pagar_incrementais(empresa_config, data_corte):
    ...
    # Vamos fazer duas passagens: Uma para INCLUSÕES, outra para ALTERAÇÕES
    # Isso garante que não perdemos nenhum cenário, caso a API trate os filtros com AND em vez de OR
    filtros = [
...
def rodar_rotina_cp_incremental():
    # Define a Janela Móvel: Pega alterações dos últimos 3 dias para garantir robustez
    # Se rodarmos toda hora, os últimos 3 dias cobrem possíveis falhas de fim de semana
    ...
    headers_supabase = {
        ...
        "Prefer": "return=minimal, resolution=merge-duplicates" # A Mágica do UPSERT (Atualiza ou Insere)
    }
    ...
    for empresa in EMPRESAS:
        ...
        if contas_pagar:
            try:
                ...
                print(f"✅ UPSERT (Atualizado/Inserido): {len(contas_pagar)} registros novos/alterados de {empresa['empresa']}")
                total_processado += len(contas_pagar)
            except Exception as e:
                print(f"❌ Erro ao enviar Contas a Pagar da empresa {empresa['empresa']}: {e}")
        else:
            print(f"Nenhuma novidade encontrada para {empresa['empresa']} desde {data_corte}.")

    print(f"\nFIM DA ROTINA INCREMENTAL! Total UPSERT: {total_processado} registros.")
